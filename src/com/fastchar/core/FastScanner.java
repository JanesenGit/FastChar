...
public final class FastScanner {
    ...
    void coreScanner(FastEngine engine) throws Exception{
        List<Class> localClasses = getLocalClasses(FastChar.getPath().getClassRootPath(),
                new File(FastChar.getPath().getClassRootPath()));
        for (Class localClass : localClasses) {
            if (IFastWeb.class.isAssignableFrom(localClass)) {
                engine.getWebs().addFastWeb(localClass);
            }
        }
        scannerLib(engine);
    }
    ...
    private void scannerSrc() throws Exception {
        ...
        while (resources.hasMoreElements()) {
            ...
            List<Class> localClasses = getLocalClasses(path, file);
            for (Class localClass : localClasses) {
                notifyAccepter(localClass);
            }
        }
    ...
    private void scannerWeb() throws Exception {
        ...
        List<File> webFiles = getWebFiles(new File(FastChar.getPath().getWebRootPath()));
        Collections.sort(webFiles, new Comparator<File>() {
            @Override
        ...
        for (File webFile : webFiles) {
            notifyAccepter(webFile);
        }
    }

    private void scannerLib(FastEngine engine) throws Exception {
        ClassLoader classLoader = Thread.currentThread().getContextClassLoader();
        Enumeration<URL> resources = classLoader.getResources("");
        while (resources.hasMoreElements()) {
        ...
        for (ScannerJar jar : jars) {
            ...
            while (jarEntries.hasMoreElements()) {
                ...
                for (String exclude : excludes) {
                    if (jarRealName.startsWith(exclude)) {
                        isExclude = true;
                        break;
                    }
                }
    ...
    private List<Class> getLocalClasses(String path,File parentFile) {
        ...
        for (File file : files) {
            if (file.isDirectory()) {
                allClass.addAll(getLocalClasses(path,file));
            } else {
                Class convertClass = convertClass(path,file);
                if (convertClass != null) {
                    allClass.add(convertClass);
                }
            }
        }
        return allClass;
    ...
    private List<File> getWebFiles(File parentFile) {
        ...
        File[] files = parentFile.listFiles(new FileFilter() {
            ...
            public boolean accept(File pathname) {
                if (pathname.getName().toLowerCase().endsWith(".class")
                        ...
                        || pathname.getName().toLowerCase().startsWith(".")
                        || pathname.isHidden()) {
                    return false;
                }
                return true;
        ...
        for (File file : files) {
            ...
            if (file.isDirectory()) {
                allFile.addAll(getWebFiles(file));
            } else {
                allFile.add(file);
            }
        }
    ...
    private void saveJarEntry(InputStream inputStream, JarEntry jarEntry,
        ...
        try {
            ...
            if (file.exists()) {
                if (!checkIsModified(jarEntryName, version)) {
                    return;
                }
            }
