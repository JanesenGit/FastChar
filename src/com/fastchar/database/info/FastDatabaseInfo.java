...
public class FastDatabaseInfo extends FastBaseInfo {
    ...
    public DataSource getDataSource() {
        return FastChar.getOverrides().singleInstance("DataSource" + getName(),
                IFastDataSourceProvider.class).getDataSource(this);
    }

    public IFastDatabaseOperateProvider getOperate() {
        if (FastStringUtils.isEmpty(getType())) {
            return null;
        }
        return FastChar.getOverrides().newInstance(IFastDatabaseOperateProvider.class, getType());
    }
    ...
    public String toUrl() {
        ...
        String url = null;
        if (isMySql()) {
            url = "jdbc:mysql://" + getHost() + ":" + getPort() + "/" + getName() + "?rewriteBatchedStatements=true" +
                    "&useUnicode=true" +
